...
| `P3-PR03` | `RUNNING` | P3-PR02, P1-PR03 | Executar Codex quota-session pelo terminal gerenciado sem extrair credencial/API | PR #13; candidate `31d2104` + hardenings `e4243ba`/`4b197b0` na base `1d40eb7`; quota ChatGPT confirmada por linha exata no preflight ConPTY, execução real com stream ao vivo/cwd/identidades, metadata efetiva da CLI `0.147.0`, argumento sensível redigido, snapshot pré-await da invocação, falha fechada sem API/fallback e `npm test` `155/155`; review `GREEN_CANDIDATE`, correção precisa publicação/revalidação |
