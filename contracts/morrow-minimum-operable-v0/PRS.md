...
| `P3-PR04` | `RUNNING` | P3-PR03 | Provar multiplexing, cleanup, timeout/cancel e colisões sob múltiplas sessões | PR #14 `RED_MATERIAL`: candidate `ff744d2` invalidado por hard timeouts e assertion `remove_pty_baton` do estado global não sincronizado do addon; corrigir com host nativo por sessão, contraprovar e revalidar antes de integrar |
