...
- `A-001` substitui somente o gate externo indisponível de P4-PR02 por revisão local independente de segurança, sem declarar equivalência entre as duas provas;
...
| `P2-PR06` | `PROVEN` | P2-PR05 | Implementar reconnect, retry idempotente, fila, checkpoint e estado online/offline | PR #10; recovery durável, replay/sequence por hashes, liveness fail-closed, kill/restart sem duplicação, 130 testes; review de segurança GREEN |
...
| `P3-PR04` | `PROVEN` | P3-PR03 | Provar multiplexing, cleanup, timeout/cancel e colisões sob múltiplas sessões | PR #14 integrada em `d4ccc73`; addon isolado por sessão/host e IPC fail-closed; saída simultânea, crash contido e protocolo inválido contraprovados; pós-merge ConPTY 11/11, suíte 164/164 e zero órfãos Morrow |
...
| `P4-PR02` | `PROVEN` | P4-PR01 | Implementar Stream Redactor, política de retenção e transcript persistente | PR de produto #18 integrada em `3738d7877cc1613e363adee8063322eefb595528` com parents `3657a070e5dc6b1e7b78fa1804761440c55efffc` e `46167607c6f0c55a7f48eec2464a7cbda327dc22`; candidate A-001 `76a41db64343131dfc20b699bedfae491859f88b` e head integrado `46167607c6f0c55a7f48eec2464a7cbda327dc22` ancestrais; Security gate `GREEN_LOCAL_A-001`, Auditor pré-merge `MERGE_READY`, Auditor final `P4_PR02_PROVEN_READY`; `npm ci` GREEN, focused `42/42`, `npm test` `214/214`, `git diff --check` GREEN, D-013 não ocorreu, sem drift e sem path inesperado. A prova local não equivale ao Security Review externo indisponível e não cria precedente. O closure-record posterior foi integrado pela PR #19 em `a7f0f49efa630f927ac22f56d8cd0ce2032664cc`; é somente documental, não é nova unidade contratual, não reinicia A-001/regressão e não constitui P4-PR03 |
| `P4-PR03` | `PROVEN` | P4-PR02 | Implementar replay/reidratação, cursores e liveness após restart | PR #21 integrada em `b95b15a61b549eaa779b26b034cea4d57adf9e74`, parents `cd7113febd147925cc5a5ab3557cb1d2ea48d1dc` e `cf44e9b728884bcd1452a1e8e4fef0470e7d341e`; Execution Root `d63a19c2fb5da3fe8f781dba18b7abe75b9f4d7a`; Pre-merge Control Root `cf44e9b728884bcd1452a1e8e4fef0470e7d341e`; árvore equivalente ao head aprovado, sem drift; `npm ci` GREEN, focused P4-PR03 `29/29`, transcript `42/42`, primeira full `251/252` por `EBADF` isolado, retry `251/251`, `git diff --check` GREEN, `REGRESSION_VETO` `PASS`; reconciliador na base com `EXPECTED_BRANCH_CONTEXT_DIVERGENCE`; Security, Reviewer, Auditor pré-merge e Auditor final `FINAL_AUDIT_GREEN` válidos |
| `P4-PR04` | `PENDING` | P4-PR03, P3-PR04 | Expor stream/API de múltiplas sessões e provar observabilidade ponta a ponta do Worker | AUTORIZADA após closure de P4-PR03; duas sessões reais vistas ao vivo e reidratadas após restart |
