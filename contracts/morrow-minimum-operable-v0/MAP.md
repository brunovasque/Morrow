...
- O reconciliador no checkout limpo da base integrada retornou exclusivamente `BLOCKED_STATE_DIVERGENCE` / `git_branch_mismatch:phase-2/runtime-v0`; isto é `EXPECTED_BRANCH_CONTEXT_DIVERGENCE` do guard branch-aware, não drift da árvore integrada. Não há token/guard separado `REGRESSION_VETO` no repositório; os gates equivalentes foram executados.
