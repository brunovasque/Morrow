...
- D-016 RED: snapshot com checksum recalculado, placeholder artificial e `redactionCount` falso era aceito. GREEN: cada novo registro carrega proveniência mínima versionada (`hmac-sha256-v1`) vinculando digest de entrada, conteúdo, identidade do registro e contagem; a chave interna do root não expõe segredo de usuário. Snapshots antigos são aceitos em compatibilidade e recebem proveniência `legacy-v1` na reidratação.
...
- Replay GREEN: `JsonlEventLog` persiste envelope versionado com sequência por contrato e processa JSONL incrementalmente, validando gaps, reorder, duplicatas, identidade, limite, head e piso retido sem materializar o histórico; estados `ok`, `invalid`, `stale` e `future` permanecem explícitos. `replayLiveActivity` usa sequência/identidade próprios; transcript usa ordinal próprio e rejeita gaps.
...
- Provas focadas nesta execução: P4-PR03 `15/15`; transcript `42/42`; worker-recovery `21/21`; kernel/live-activity/P4-PR03/recovery combinados `52/52`. Replay proporcional de ~11,4 MB retornou `1` evento com delta de heap pós-GC de aproximadamente `151 KB`; o baseline materializador medido pelo Reviewer foi aproximadamente `82 MB` para ~29,8 MB.
