...
| `P2-PR06` | `9b96a8b` | código/teste `d2c54de`; head remoto pendente | `mvo/p2-pr06-recovery`; PR remota pendente | fila/checkpoint atômicos, online/connecting/offline por hello+heartbeat, idempotência/fingerprint/capacity, retry somente pré-efeito e bloqueio fail-closed; processo filho retoma queued uma vez; kill após efeito reidrata `execution_outcome_unknown_after_restart` sem replay; corrupção e material sensível atacados; testes focados 11/11; `npm test` 120/120; `git diff --check` verde; sem ConPTY, rede concreta, credencial ou target externo | Architect/Security Reviewer/Reviewer local: `GREEN_CANDIDATE`; revisão do head remoto pendente | `RUNNING` | 2026-08-29 |
