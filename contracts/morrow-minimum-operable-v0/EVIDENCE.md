...
| `P2-PR03` | `330bfa1` | código/teste `3b0a8be`; hardening `b8b7edc`; binding proof `d17e1eb`; trusted clock `73c353a` | [`PR #7`](https://github.com/brunovasque/Morrow/pull/7), `mvo/p2-pr03-registries`; head inicial revisado `38e65f6`, correção pendente de publicação | registries/resolver estritos, versões exatas, target não implícito e Secret Broker com handle opaco/idempotente e relógio confiável; `npm test` 73/73; `git diff --check` verde; nenhuma credencial real, dispatch, shell, rede ou target externo | Security Reviewer encontrou relógio controlável pelo chamador e o bloqueio foi corrigido com contraprova; `GREEN_CANDIDATE_AFTER_FIX` em `reviews/P2-PR03.md`; head corrigido exige confirmação remota antes de `PROVEN` | `RUNNING` | 2026-08-29 |
