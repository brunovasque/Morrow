...
- Replay GREEN: `JsonlEventLog` persiste envelope `morrow.event-log/2` com sequência, hash anterior e hash do domínio canônico por contrato; o scanner processa JSONL em chunks, aplica teto de 1 MiB por record durante a acumulação, rejeita UTF-8 inválido/malformed sem conteúdo bruto e usa Bloom filter fixo de 8 MiB para unicidade sem `Set` proporcional ao histórico. Gaps, reorder, perda, hash inválido, duplicatas, identidade, limite, head e piso retido falham fechado; estados `ok`, `invalid`, `stale` e `future` permanecem explícitos. `replayLiveActivity` usa sequência/identidade próprios; transcript usa ordinal próprio e rejeita gaps.
...
- Provas focadas nesta execução: P4-PR03 `19/19`; transcript `42/42`; worker-recovery `21/21`; live-activity `8/8`; suíte completa `233/233`. Contraprovas incluem downgrade por remoção de formato/proveniência, ordinal `[1,2]` adulterado para `[2,3]` com checksum recalculado, reorder/gap/duplicate com hash reconstituído, cursor hostil, linha UTF-8 gigante atravessando chunks e HMAC inválido. Replay isolado de 50.000 eventos (~19,5 MB), em processo separado e com `limit=1`, retornou `1` evento; após GC, RSS foi de aproximadamente `68,5 MB` para `80,6 MB` e memória externa de `8,5 MB` para `17,5 MB`, com o aumento fixo compatível com o scanner bounded, sem IDs do histórico. O baseline materializador medido pelo Reviewer foi aproximadamente `82 MB` para ~29,8 MB; a comparação é indicativa, não benchmark.
