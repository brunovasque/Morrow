...
| `P2-PR04` | `7d3e91b` | código/teste `8c68d87`; head remoto `PENDING` | `mvo/p2-pr04-routing-guards`; PR `PENDING` | configuração efetiva com precedência/proveniência; compatibilidade exata; quota/budget com reserva, ownership e idempotência; `npm test` 94/94; `git diff --check` verde; sem dispatch, processo, rede ou cobrança real | revisão adversarial corrigiu teto agregado por invocação, fingerprint canônico e replay de settlement; Architect/Security Reviewer/Reviewer local: `GREEN_CANDIDATE`; review remoto obrigatório antes de `PROVEN` | `RUNNING` | 2026-08-29 |
