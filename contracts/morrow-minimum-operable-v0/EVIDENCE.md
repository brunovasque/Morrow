...
| `P2-PR05` | `aba8770` | código/teste `cabc1db` | `mvo/p2-pr05-authenticated-dispatch`; PR remota pendente | envelope autenticado → WorkSpec/WorkAuthority/PRE_DISPATCH → routing/guards → lock/workspace → PowerShell ou AgentInstance; `npm test` 106/106; PowerShell Windows e agente processual em fixtures `.morrow` temporárias; contraprovas de auth, comando bruto, hash/target/context stale, readiness, idempotência/rebinding, lock, quota/budget, settlement e cleanup; `git diff --check` verde; sem ConPTY, persistência, credencial, rede ou target externo | achados de readiness, settlement, target binding, contexto stale e cleanup resolvidos; Architect/Security Reviewer/Reviewer local: `GREEN_CANDIDATE`; revisão do head remoto obrigatória antes de `PROVEN` | `RUNNING` | 2026-08-29 |
