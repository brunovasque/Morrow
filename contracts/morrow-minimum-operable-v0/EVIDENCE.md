...
| `P2-PR05` | `aba8770` | código/teste `cabc1db`; head inicial `c3636cb`; hardening `caf4eed` | [`PR #9`](https://github.com/brunovasque/Morrow/pull/9), `mvo/p2-pr05-authenticated-dispatch` | envelope autenticado → WorkSpec/WorkAuthority/PRE_DISPATCH → routing/guards → lock/workspace → PowerShell ou AgentInstance; `npm test` 108/108; PowerShell Windows e agente processual em fixtures `.morrow` temporárias; contraprovas de auth, comando bruto, hash/target/context stale, attachment/readiness, idempotência/rebinding/capacity, lock, quota/budget, settlement, stdin e cleanup; `git diff --check` verde; sem ConPTY, persistência, credencial, rede ou target externo | revisão remota encontrou enablement configurável, memória sem teto e stdin sem boundary; `caf4eed` corrigiu e passou 108 testes; Architect/Security Reviewer/Reviewer: `GREEN_CANDIDATE`; novo head remoto ainda precisa ser conferido antes de `PROVEN` | `RUNNING` | 2026-08-29 |
