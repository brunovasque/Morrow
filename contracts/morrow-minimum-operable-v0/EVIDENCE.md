...
| `P2-PR03` | `330bfa1` | código/teste `3b0a8be`; hardening `b8b7edc`; binding proof `d17e1eb` | `mvo/p2-pr03-registries`; PR remota pendente de publicação | registries/resolver estritos, versões exatas, target não implícito e Secret Broker com handle opaco/idempotente; `npm test` 73/73; `git diff --check` verde; nenhuma credencial real, dispatch, shell, rede ou target externo | revisão interna de arquitetura/segurança `GREEN_CANDIDATE` em `reviews/P2-PR03.md`; revisão remota obrigatória antes de `PROVEN` | `RUNNING` | 2026-08-28 |
