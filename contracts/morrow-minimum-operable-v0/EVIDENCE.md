...
| `P2-PR04` | `7d3e91b` | código/teste `8c68d87`; hardening `1180dc6`, `240c646`; head remoto inicial `bc527ae`; corrigido `PENDING` | [`PR #8`](https://github.com/brunovasque/Morrow/pull/8), `mvo/p2-pr04-routing-guards` | configuração efetiva com precedência/proveniência; quota/budget com reserva, ownership, idempotência e chaves sem colisão; `npm test` 96/96; `git diff --check` verde; sem dispatch, processo, rede ou cobrança real | revisão remota encontrou colisão de IDs compostos e spoof de prefixo de erro; ambos corrigidos localmente com contraprovas; `GREEN_CANDIDATE_CORRECTED`; novo head remoto obrigatório antes de `PROVEN` | `RUNNING` | 2026-08-29 |
