...
| `P2-PR06` | `9b96a8b` | código/teste `d2c54de`; head inicial `3669c8a`; hardening `9b02ba5`; prova `bfecf40` | [`PR #10`](https://github.com/brunovasque/Morrow/pull/10), `mvo/p2-pr06-recovery`; atualização remota pendente | fila/checkpoint atômicos, online/connecting/offline por hello+heartbeat, idempotência/fingerprint/capacity, retry somente pré-efeito e bloqueio fail-closed; processo filho retoma queued uma vez; kill após efeito reidrata `execution_outcome_unknown_after_restart` sem replay; segunda instância, target incerto, corrupção/tamanho e material sensível atacados; testes focados 13/13; `npm test` 122/122; `git diff --check` verde; sem ConPTY, rede concreta, credencial ou target externo | revisão do head inicial encontrou ausência de dono único, continuidade no mesmo target incerto e leitura antes do limite; `9b02ba5` + `bfecf40` corrigem/provam; Architect/Security Reviewer/Reviewer: `GREEN_CANDIDATE`, revalidação remota pendente | `RUNNING` | 2026-08-29 |
