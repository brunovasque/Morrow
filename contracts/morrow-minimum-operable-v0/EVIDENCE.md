...
- Replay GREEN: `JsonlEventLog` persiste envelope `morrow.event-log/3` com sequência, tag autenticada anterior e HMAC-SHA-256 do domínio canônico por contrato/stream; a chave própria é derivada com domain separation (`morrow.event-log/root-key/v1`), não é compartilhada com transcript e nunca aparece no JSONL ou retorno público. O scanner processa JSONL em chunks, aplica teto de 1 MiB por record durante a acumulação, rejeita UTF-8 inválido/malformed sem conteúdo bruto e usa Bloom filter fixo de 8 MiB para unicidade sem `Set` proporcional ao histórico. Gaps, reorder, perda, rewrite integral, hash/tag inválido, duplicatas, identidade, limite, head e piso retido falham fechado; ausência de log com chave já existente não vira log vazio; estados `ok`, `invalid`, `stale` e `future` permanecem explícitos. `replayLiveActivity` usa sequência/identidade próprios; transcript usa ordinal próprio e rejeita gaps.
...
- Provas focadas nesta execução: P4-PR03 `22/22`; transcript `42/42`; worker-recovery `21/21`; live-activity `8/8`; suíte completa `236/236`. RED no candidate `54ba8ea` foi o rewrite integral com alteração/remoção/reindex/inserção e SHA público refeito, aceito pela cadeia não autenticada; GREEN rejeita esses cenários pela tag HMAC. As contraprovas adicionais cobrem downgrade por formato, alteração isolada de sequence/eventId/contrato/tipo/payload/tag, key ausente/corrompida/diferente, cópia para outro path, cursor hostil, linha UTF-8 gigante atravessando chunks e HMAC inválido. Medição final isolada de 50.000 eventos (~19,5 MB), em processo separado e com `limit=1`, retornou `1` evento; após GC, RSS foi de aproximadamente `68,8 MB` para `81,4 MB` e memória externa de `8,6 MB` para `17,6 MB`, com o aumento fixo compatível com o scanner bounded, sem IDs do histórico. O baseline materializador medido pelo Reviewer foi aproximadamente `82 MB` para ~29,8 MB; a comparação é indicativa, não benchmark.
