...
- Pass 1 independente por papéis: `PENDING`
- Pass 2 adversarial: `PENDING`
- perguntas de destino abertas: `0`
- bloqueio atual: aprovação do draft pelo dono + passes independentes em `P0-PR02`
...
| `Q-09` | Architect | terminal | Qual biblioteca/estratégia ConPTY será usada? | Dependência nativa e segurança | state-of-art required | `ROUTE_DECISION` | P3-PR01 mede opções e registra ADR; não altera destino. |
...
|---|---|---|
| Contract Engineer | destino, aceitação, exclusões, decisões do dono, linguagem observável | `PENDING` |
| Architect | fronteiras control plane/Worker/UI/gateway/Nexus, dependências e substituibilidade | `PENDING` |
| Test Designer | contraprovas, restart, concorrência, isolamento, off-PC e aceite fora do harness | `PENDING` |
| Security Reviewer | shell/ConPTY, rede, identidade, replay, secrets, target escape e logs | `PENDING` |
| Reviewer | coerência interna, duplicação, lacunas e regressões da base | `PENDING` |
| Acceptance | se o estado-alvo pode ser observado por operador não desenvolvedor | `PENDING` |
