...
- `active_subaction`: `P4_PR03_EXECUTOR_HANDOFF`
...
- `next_authorized_actor`: `Executor P4-PR03 deve executar a TASK PRE_DISPATCH na branch mvo/p4-pr03-replay-rehydration; implementação e correções permanecem na mesma sessão`

## Próxima ação exata

P4-PR02 está `PROVEN` após o merge commit `3738d7877cc1613e363adee8063322eefb595528` da PR #18, com Auditor pré-merge `MERGE_READY`, regressão pós-merge verde e Auditor final `P4_PR02_PROVEN_READY`. A prova local `GREEN_LOCAL_A-001` não equivale ao Security Review externo indisponível e não cria precedente. A closure-record foi integrada pela PR #19 no merge `a7f0f49efa630f927ac22f56d8cd0ce2032664cc`. P4-PR03 está `RUNNING`, com base fixada em `cd7113febd147925cc5a5ab3557cb1d2ea48d1dc`, na branch `mvo/p4-pr03-replay-rehydration`, e recebeu PRE_DISPATCH para execução; nenhum código foi implementado nesta etapa.
